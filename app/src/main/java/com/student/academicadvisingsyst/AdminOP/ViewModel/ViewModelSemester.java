...
import com.student.academicadvisingsyst.AdminOP.Repository.RepositorySemester;
...
public class ViewModelSemester extends ViewModel {
    public MutableLiveData<List<SemesterNew>> mutableLiveData = new MutableLiveData<>();


    private RepositorySemester repository;
    public void init(){

        if(mutableLiveData != null){
            return;
        }
        repository = RepositorySemester.getInstance();
...
//
//        RepositorySemester.getInstance().createSemester(initDataBind(addStudentBinding)).enqueue(new Callback<Student>() {
//            @Override
...
//    return res;
//    }
    public void getSemestr (Context context, ProgressBar bar){
        RepositorySemester.getInstance().getStudentData().enqueue(new Callback<List<SemesterNew>>() {
            @Override
            public void onResponse(Call<List<SemesterNew>> call, Response<List<SemesterNew>> response) {
            ...
            @Override
            public void onFailure(Call<List<SemesterNew>> call, Throwable t) {
